    ...
    const fetchData = async () => {
      try {
        ...
        if (selectedDoctor) {
          ...
          // Fetch other doctors from same department
          const deptRes = await fetch(`/api/doctors?departmentId=${selectedDoctor.departmentId}`)
          const deptDoctors = await deptRes.json()
      ...
      <section className="w-full py-16 px-4 bg-gradient-to-r from-primary/10 to-accent/10">
        <div className="container mx-auto">
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-4">
            <Link href="/doctors" className="text-primary hover:text-primary/80 font-medium flex items-center gap-2">
              ← Back to Doctors
            </Link>
          </motion.div>
        </div>
      ...
      <section className="w-full py-20 px-4">
        <div className="container mx-auto max-w-4xl">
          ...
          >
            ...
            <div className="md:col-span-2">
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>
                ...
                <p className="text-2xl text-primary font-medium mb-6">{doctor.specialty}</p>

                <div className="space-y-4 mb-8">
                  <div className="flex items-center gap-3 text-lg">
                    <Briefcase size={24} className="text-primary" />
                    <span className="text-foreground">{doctor.experienceYears}+ years of experience</span>
                  </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-muted/50 p-4 rounded-lg border border-border">
                    <p className="text-sm text-muted-foreground mb-3">
                      <strong>About:</strong> Dr. {doctor.name} is a highly skilled {doctor.specialty} with extensive
                      experience in providing comprehensive medical care.
                    </p>
                  </div>
                </div>
          ...
          {departmentDoctors.length > 0 && (
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} className="mt-16">
              <h2 className="text-3xl font-bold text-foreground mb-8">Other Doctors in {doctor.specialty}</h2>
                      ...
                      <Card className="overflow-hidden border-0 shadow-lg hover:shadow-2xl transition-shadow h-full flex flex-col">
                        ...
                        <div className="p-4 flex-1 flex flex-col">
                          ...
                          <Button
                            ...
                            asChild
                          >
                            <Link href={`/appointment?doctorId=${doc.id}`}>Book</Link>
                          </Button>
                        </div>
                      </Card>
                    </Link>
